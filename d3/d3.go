...
func ParseAll(input string) int {
	var p parser
	nodes := make([]Node, 0)
	p.Init()
...
func (p *parser) ParseNext(c int32) {
	...
	handler, ok := statusMap[p.stat]
	if !ok {
		panic(fmt.Sprintf("invalid status %s", p.stat))
	}
	p.stat = handler(c, p.n)
}
...
func (p *parser) GetNode() (Node, error) {
	if p.stat != fin {
		return Node{}, NodeNotComplete
	}
	return *p.n, nil
}
...
var NodeNotComplete error = errors.New("NodeNotComplete")
